...
```
NumPy ist eine Bibliothek, die eine einfache Handhabung
von Vektoren, Matrizen oder generell großen mehrdimensionalen Arrays ermöglicht.
Neben den Datenstrukturen bietet NumPy auch effizient implementierte Funktionen
für numerische Berechnungen an.
```

##### Interpretation des Code Beispiels:

Zuerst wird das numpy array age_in_years angelegt und mit np.where alle Einträge selektiert die kleiner als 20 sind. Andere werden auf 0 gesetzt.
...
```
Pandas ist ein Paket zur Datenanalyse.
Es bietet insbesondere Datenstrukturen und Operationen für die Bearbeitung
von numerischen Tabellen und Zeitreihen.
```
...
```haskell
quickSort []     = []                               -- Die leere liste ist bereits sortiert
-- x:xs nimmst das erste Element aus der Liste und nennt es x, xs ist dann der Rest der Liste ohne x
quickSort (x:xs) = quickSort [a | a <- xs, a < x]   -- Sortiere die linke Seite
...
3. Was versteht man unter dem Precision- und Recall Tradeoff? Interpretieren Sie hiefür das Schaubild aus der Vorlesung.

![](images/precision-recall-2-curves.png){ width=150px }
...
![](images/health_data.png){ width=150px }
...
![](images/training_val.png){ width=150px }
...
- In welchen Bereich underfittet bzw. overfitted das Modell? Geben Sie dies im Diagramm mit "Overfitting" und "Underfitting" an.

## b) AlexNet

Auf dem Bild ist die Architektur von AlexNet zu sehen. Ergänzen Sie den unten stehenden TensorFlow Code um AlexNet in Keras zu implementieren.

![](images/alex_net){ width=150px }
...
![Schaubild A](images/03_decision_trees_with_bagging.png){ width=150px }
